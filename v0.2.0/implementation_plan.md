...
## v0.2.0で満たしたい要件
1. 最小限の「運用オーケストレーション」をMCPサーバ内で提供
2. 状態モデルの拡張（タスク・フェーズ・成果物・失敗理由の明文化）
3. 低コストな観測性（ログ要約・集計・再現性向上）
4. セッション切替時の引き継ぎ強化（コンテキスト枯渇対策）
5. セッション内の自走性向上（手動トリガーで自己再開できる）
6. `.rules`によるワークフロー制御の改善
7. 消費トークンの低減
