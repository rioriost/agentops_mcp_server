# Draft for 0.6.0: redesign canonical identifiers around session, transaction, and event logs
...
For 0.6.0, redesign canonical identity handling so that the server explicitly manages:
...
For 0.6.0, `seq` should remain sufficient.
...
0.6.0 should define a clear compatibility policy for those cases.
...
## New writes
All new canonical writes after 0.6.0 should use the new layered identity model.
...
0.6.0 should be considered complete when:
...
The core proposal for 0.6.0 is to make canonical runtime identity explicit and layered:
